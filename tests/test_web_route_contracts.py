...
def _route_entries() -> list[tuple[str, str, str, bool]]:
    app = create_app()
...
def _route_map() -> dict[tuple[str, str], tuple[str, bool]]:
    return {
        (method, path): (endpoint_name, include_in_schema)
        for method, path, endpoint_name, include_in_schema in _route_entries()
    }
...
def test_files_route_contract_is_stable():
    ...
    for key, expected_endpoint in FILES_ROUTE_CONTRACT.items():
        ...
        endpoint_name, include_in_schema = routes[key]
...
def test_app_has_no_duplicate_method_path_routes():
    ...
    for method, path, _endpoint_name, _include_in_schema in _route_entries():
        key = (method, path)
        if key in seen:
